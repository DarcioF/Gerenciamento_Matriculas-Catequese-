...
<?php
    ...
    ?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800"> <i class="fas fa-cross"></i>   Relatório Catequista</h1>
          </div>
             <?php         
                  if(isset($_GET['res'])){
                    $s = $_GET['res'];
              ...
              <form  method="GET" class="form-group col-md-6" action="buscar_catequista.php" >
  ...
  <?php
    ...
    $result = $sa -> listar_Catequista($conn); 
    if ($result) {      
        while($row = $result->fetch(PDO::FETCH_OBJ)){
              if (($pesquisa==$row -> nomeC)||($pesquisa==$row -> cpf)|| ($pesquisa== $row -> local)||($pesquisa== $row -> telefone)||($pesquisa== $row -> email) ||($pesquisa== $row -> descricao)) {
                echo '<tr>';
          echo  '<td >'.$row -> nomeC . '</td>'. 
              ...
              <td> <a href="form_up_catequista.php?id='.
               $row -> idCatequista.'&idSacramento='.$row-> idSacramento.'"><i class=" far fa-edit"></i></a>'.'</td>
              <td><a value="id'.
               $row -> idCatequista.'"><a href="deletar_catequista.php?id='.$row -> idCatequista.'"><button class="dropdown-item" onclick=" return confirm('."'Deseja deletar o registo?'".')"><i class="fa fa-times"></i></button></a></td>';
        echo '</tr>';
        $cont++;
              }
